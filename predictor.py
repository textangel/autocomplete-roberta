...
class PythonPredictor:
    ...
    def post_process(self,prediction, ):
        # if prediction[-1].strip() in punctuation:
        #     return (prediction[2], prediction[1] * 0.001)
    ...
    def predict(self, payload):
        # while 
        predictions = self.model.fill_mask(payload["text"] + " <mask>", topk=20)
        # if payload["text"][-1] in punctuation:
        #     pp = self.post_process
        return sorted([self.post_process(prediction) for prediction in predictions if self.post_process(prediction)], key= lambda x:-x[1])[:10]
